...
#define CHECK_H_INCLUDED
 ...
 int counter_R,counter_N,counter_B,counter_P,counter_r,counter_n,counter_b,counter_p;//we should make it !
    ...
    //inverse in white peaces
    // we want to return those values to make us could use check mate
...
// white
int king_w_place(char kind,char colors,int color,int x,int y){
    int flag=0;
    int a;
     if(colors=='w' && kind=='k'){
        if(color==1){
 ...
 //for queen
 int e[2];
 int queen_w_place(char kind,char colors,int color){
     int a;
     if(colors=='w' && kind=='q'){
        if(color==1){
            for(int i=0;i<8;i++){
 ...
 //rook
 int rook_w_place(char kind,char colors,int color,counter_r){
     int rook_arr[4]={0};
     int a;
     k=0,h=k+1;
      if(colors=='w' && kind=='r'){
  ...
  //for knight
  int knight_w_place(char kind,char colors,int color,counter_n){
      int knight_arr[4]={0};
      int flag=0;
      k=0,h=k+1;
        if(colors=='w' && kind=='n'){
  ...
  int bishop_w_place(char kind,char colors,int color,counter_b){
      int bishop_arr[4]={0};
      int k=0,h=k+1;
          if(colors=='w' && kind=='b'){
            if(counter_b==2){
  ...
  int pawns_w_place(char kind,char colors,int color,counter_p){
      int pawns_arr[8]={0};
          int t=0,r=t+1;
          if(colors=='w' && kind=='p'){
                for(int k=1;k<=8;k++){
